    ...
    if (game) {
      const sketch = (p) => {
        ...
        p.setup = () => {
          ...
          p.textSize(20);
          p.noStroke();
          backgroundColor = p.color("black"); // Varsayılan arka plan rengi
        ...
        p.draw = () => {
          ...
          if (g) {
            ...
            if (!p.select("img")) {
              const gif = p.createImg("hugo.gif", "HUGO GIF"); // GIF dosyasını ekleyin
              gif.position(gifX, gifY); // GIF'in konumunu ayarla
              gif.size(gifWidth, gifHeight); // GIF'in boyutunu ayarla
              gif.style("z-index", "10"); // Canvas üzerinde görünmesi için z-index ayarı
            }
            const textX = gifX + 20;
            p.fill(255);
            p.textAlign(p.LEFT);
                ...
                <div className="dream-btn-group" data-wow-delay="0.4s">
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      setGame(true);  // Oyun bileşenini aktif et
                      window.scrollTo({
                        top: document.body.scrollHeight,  // Sayfanın en altına git
                  ...
                  <a href="https://t.me/kspr_home_bot" className="btn more-btn" target="_blank" rel="noopener noreferrer">
                    {link2}
                  </a>
                </div>
              </div>
